...
namespace Chroma
{
  namespace LinOpSysSolverQUDATwistedCloverEnv
  {
    ...
    {
      //! Name to be used
      const std::string name("QUDA_TWISTED_CLOVER_INVERTER");
    ...
    {
      return new LinOpSysSolverQUDATwistedClover(A, state,SysSolverQUDACloverParams(xml_in, path));
    }
...
#ifdef _PDF_
  LinOpSysSolverQUDATwistedClover::qudaInvert(const CloverTermT<T, U>::Type_t& clover,
				       const CloverTermT<T, U>::Type_t& invclov,
				       const T& chi_s,
				       T& psi_s) const{
#else				       
  LinOpSysSolverQUDATwistedClover::qudaInvert(const CloverTermT<T, U>& clover,
